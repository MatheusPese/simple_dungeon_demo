	...
	for x in range(dungeon_h_size):
		for z in range(dungeon_v_size):
			if get_node("Dungeon_gridmap").get_cell_item(x,0,z) == 0: # Wall
				...
				wall.translation = Vector3(x * cell_size.x + 1.0, 1.0, z * cell_size.x + 1.0)
				add_child(wall)
			elif get_node("Dungeon_gridmap").get_cell_item(x,0,z) == 1: # Key
				var key = Key.instance()
				key.translation = Vector3(x*cell_size.x + 1.0, 1.0, z * cell_size.x + 1.0)
				add_child(key)
				...
				door.set_translation(Vector3(x*cell_size.x + 1.0,1.0,z*cell_size.x + 1.0))
